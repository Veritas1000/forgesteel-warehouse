...
class PatreonApi:
    ...
    def generate_authorize_url(self, redirect_uri, state):
        ...
        url = f"https://www.patreon.com/oauth2/authorize?{urlencode(params, quote_via=quote)}"

        if log.getEffectiveLevel() < logging.DEBUG:
            log.debug(f"Authorization request URL is [{url}]")
    ...
    def get_token(self, auth_code, redirect_uri):
        ...
        if log.getEffectiveLevel() < logging.DEBUG:
            log.debug(f"Got an access_token [{access_token[0:5]}...]")
            log.debug(f"Token has scope(s) [{tokens["scope"]}]")
    ...
    def _parse_identity_response(self, identity_json):
        ...
        if identity_json is not None and "data" in identity_json:
            ...
            if (
                "attributes" in identity_json["data"]
                and "email" in identity_json["data"]["attributes"]
            ):
        ...
        if identity_json is not None and "included" in identity_json:
            ...
            tiers = list(
                filter(lambda inc: inc["type"] == "tier", identity_json["included"])
            )
            tier_map = dict(
                map(lambda tier: (tier["id"], tier["attributes"]["title"]), tiers)
            )

            forgesteel_state = self._get_patron_state(
                self.FORGESTEEL_CAMPAIGN_ID, memberships, tier_map
            )
            mcdm_state = self._get_patron_state(
                self.MCDM_CAMPAIGN_ID, memberships, tier_map
            )
        ...
        for membership in memberships:
            if membership["relationships"]["campaign"]["data"]["id"] == campaign_id:
                ...
                if patron_state.patron:
                    tiers = list(
                        t["id"]
                        for t in membership["relationships"][
                            "currently_entitled_tiers"
                        ]["data"]
                    )
                    patron_state.tiers = list(
                        map(lambda t: PatronTier(id=t, title=all_tiers.get(t)), tiers)
                    )
                    patron_state.tier_cents = membership["attributes"][
                        "currently_entitled_amount_cents"
                    ]
                    patron_state.start = datetime.fromisoformat(
