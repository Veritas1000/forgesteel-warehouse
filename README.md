...
- *(Technically optional but HIGHLY recommended)* Reverse proxy with SSL setup and configured
...
The initial bootstrap process creates a default configuration file along with the default user and api key. This `config.json` file can be found alongside the sqlite database in the data directory. Configuration of the app can be managed through updates to this file *or* through environment variables passed to the container. Unless otherwise noted, the key in `config.json` is the same as the env var name.

| Variable name | Default | Description |
| --- | --- | --- |
| `LOG_LEVEL` | `ERROR` | Sets the root log level. Possible values are `DEBUG`, `INFO`, `WARNING`, `ERROR`. |
| `DATABASE_URI` | `sqlite:////data/db.sqlite` | Defaults to the sqlite database created in the container data volume. |
| `FSW_CONFIG_PATH` | `/data/config.json` | Path to the config json file to load. |
| `JWT_SECRET_KEY` | *<randomly generated>* | **Advanced config** Randomly generated by the bootstrap process. Used to securely sign session JWTs. |
| `SECRET_KEY` | *<randomly generated>* | **Advanced config** Randomly generated by the bootstrap process. Used by the web server to secure session data. |
| `COOKIE_SECURE` | `True` | Set the session and auth cookie 'secure' attribute. See [flask](https://flask.palletsprojects.com/en/stable/config/#SESSION_COOKIE_SECURE) and [flask-jwt](https://flask-jwt-extended.readthedocs.io/en/stable/options.html#jwt-cookie-secure) docs for reference. |
| `COOKIE_SAMESITE` | `Lax` |  Set the session and auth cookie 'SameSite' attribute. See [flask](https://flask.palletsprojects.com/en/stable/config/#SESSION_COOKIE_SAMESITE) and [flask-jwt](https://flask-jwt-extended.readthedocs.io/en/stable/options.html#JWT_COOKIE_SAMESITE) docs for reference. |
| `COOKIE_DOMAIN` | *None* |  Set the session and auth cookie `Domain` attribute. See [flask](https://flask.palletsprojects.com/en/stable/config/#SESSION_COOKIE_DOMAIN) and [flask-jwt](https://flask-jwt-extended.readthedocs.io/en/stable/options.html#JWT_COOKIE_DOMAIN) docs for reference. |

In addition, setting an environment variable `SKIP_BOOTSTRAP` to `True` will skip the user and config file creation process, if your particular deployment would benefit from that.
