...
def test_connect_returns_200_with_good_token(client, test_user_token):
    response = client.get('/connect', headers=[['Authorization', f"Bearer {test_user_token}"]])
    assert response.status_code == 200
    assert response.json['access_token'] is not None

def test_protected_with_token(client, test_user_token):
    authResponse = client.get('/connect', headers=[['Authorization', f"Bearer {test_user_token}"]])
